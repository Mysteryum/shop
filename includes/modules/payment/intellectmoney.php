<?php
/* -----------------------------------------------------------------------------------------
   $Id: intellectmoney.php 998 2007/02/07 13:24:46 VaM $
...
// class constructor
    function intellectmoney() {
      global $order;

      $this->code = 'intellectmoney';
      $this->title = MODULE_PAYMENT_INTELLECTMONEY_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_INTELLECTMONEY_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_INTELLECTMONEY_TEXT_ADMIN_DESCRIPTION;
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_INTELLECTMONEY_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_INTELLECTMONEY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_intellectmoney_id'])) {
        $order_id = substr($_SESSION['cart_intellectmoney_id'], strpos($_SESSION['cart_intellectmoney_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_intellectmoney_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_intellectmoney_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_intellectmoney_id'], 0, strlen($_SESSION['cartID']))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_intellectmoney_id'] = $_SESSION['cartID'] . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_INTELLECTMONEY_TEXT_DESCRIPTION);
    }
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_INTELLECTMONEY_STATUS'");
