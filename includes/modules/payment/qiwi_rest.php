...
// class constructor
    public function qiwi_rest() {
      ...
      $this->code = 'qiwi_rest';
      $this->title = MODULE_PAYMENT_QIWI_REST_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_QIWI_REST_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_QIWI_REST_TEXT_ADMIN_DESCRIPTION;
...
// class methods
    public function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_QIWI_REST_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_QIWI_REST_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
	...
	function selection() {
      ...
      if (isset($_SESSION['cart_qiwi_rest_id'])) {
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_qiwi_rest_id']);
        }
      }
    ...
    public function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_qiwi_rest_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_qiwi_rest_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_qiwi_rest_id'] = $cartID . '-' . $insert_id;
        }
...
// Выписываем qiwi счёт для покупателя
		...
		$shop_id = MODULE_PAYMENT_QIWI_REST_ID;
		$order_id = substr($_SESSION['cart_qiwi_rest_id'], strpos($_SESSION['cart_qiwi_rest_id'], '-')+1);		
		$api_id = MODULE_PAYMENT_QIWI_REST_API_ID;
		$notify_pass = MODULE_PAYMENT_QIWI_REST_NOTIFY_PASS;
		...
		$currency_code = 'RUB';
		$summ = number_format($order->info['total'],2,'.','');
		...
		curl_close($ch);
	      ...
	      vam_db_query("INSERT INTO ".TABLE_PERSONS." (orders_id, name, address) VALUES ('" . vam_db_prepare_input((int)substr($_SESSION['cart_qiwi_rest_id'], strpos($_SESSION['cart_qiwi_rest_id'], '-')+1)) . "', '" . vam_db_prepare_input($_SESSION['kvit_name']) . "', '" . vam_db_prepare_input($_SESSION['qiwi_telephone']) ."')");
      ...
      return array('title' => MODULE_PAYMENT_QIWI_REST_TEXT_DESCRIPTION);
    }
...
// unregister session variables used during checkout
    ...
    public function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_QIWI_REST_STATUS'");
    ...
    public function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_QIWI_REST_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_QIWI_REST_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_QIWI_REST_ID', '', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_QIWI_REST_API_ID', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_QIWI_REST_NOTIFY_PASS', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_QIWI_REST_SORT_ORDER', '0', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_QIWI_REST_ZONE', '0', '6', '7', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_QIWI_REST_ORDER_STATUS_ID', '0', '6', '9', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
