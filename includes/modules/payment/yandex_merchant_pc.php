<?php
/* -----------------------------------------------------------------------------------------
   $Id: yandex_merchant_pc.php 998 2007/02/07 13:24:46 VaM $
  ...
  class yandex_merchant_pc {
    var $code, $title, $description, $enabled;

// class constructor
    function yandex_merchant_pc() {
      global $order;

      $this->code = 'yandex_merchant_pc';
      $this->title = MODULE_PAYMENT_YANDEX_MERCHANT_PC_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_YANDEX_MERCHANT_PC_TEXT_PUBLIC_TITLE;
      $this->icon = DIR_WS_ICONS . 'yandex.png';
      $this->description = MODULE_PAYMENT_YANDEX_MERCHANT_PC_TEXT_ADMIN_DESCRIPTION;
      $this->sort_order = MODULE_PAYMENT_YANDEX_MERCHANT_PC_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_YANDEX_MERCHANT_PC_STATUS == 'True') ? true : false);

      ((MODULE_PAYMENT_YANDEX_MERCHANT_PC_TEST == 'test') ? $this->form_action_url = 'https://demomoney.yandex.ru/eshop.xml' : $this->form_action_url = 'https://money.yandex.ru/eshop.xml');
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_YANDEX_MERCHANT_PC_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_YANDEX_MERCHANT_PC_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_yandex_id_pc'])) {
        $order_id = substr($_SESSION['cart_yandex_id_pc'], strpos($_SESSION['cart_yandex_id_pc'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_yandex_id_pc']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_yandex_id_pc'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_yandex_id_pc'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_yandex_id_pc'] = $cartID . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_YANDEX_MERCHANT_PC_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = vam_draw_hidden_field('shopId', MODULE_PAYMENT_YANDEX_MERCHANT_PC_SHOP_ID) .
                               vam_draw_hidden_field('scid', MODULE_PAYMENT_YANDEX_MERCHANT_PC_SCID) .
                               vam_draw_hidden_field('sum', $order_sum) . 
                               vam_draw_hidden_field('customerNumber', $order->customer['id']) .
                               vam_draw_hidden_field('orderNumber', substr($_SESSION['cart_yandex_id_pc'], strpos($_SESSION['cart_yandex_id_pc'], '-')+1)) .
                               ...
                               vam_draw_hidden_field('paymentType', MODULE_PAYMENT_YANDEX_MERCHANT_PC_PAYMENT_TYPE);
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_YANDEX_MERCHANT_PC_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_YANDEX_MERCHANT_PC_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YANDEX_MERCHANT_PC_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YANDEX_MERCHANT_PC_SHOP_ID', '', '6', '3', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YANDEX_MERCHANT_PC_SCID', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YANDEX_MERCHANT_PC_SECRET_KEY', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_YANDEX_MERCHANT_PC_SORT_ORDER', '0', '8', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_YANDEX_MERCHANT_PC_ZONE', '0', '6', '7', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_YANDEX_MERCHANT_PC_ORDER_STATUS_ID', '0', '6', '8', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_YANDEX_MERCHANT_PC_TEST', 'test', '6', '9', 'vam_cfg_select_option(array(\'test\', \'production\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_YANDEX_MERCHANT_PC_PAYMENT_TYPE', 'PC', '6', '10', 'vam_cfg_select_option(array(\'PC\', \'AC\', \'MC\', \'GP\', \'WM\', \'SB\', \'MP\', \'AB\'), ', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_YANDEX_MERCHANT_PC_STATUS', 'MODULE_PAYMENT_YANDEX_MERCHANT_PC_ALLOWED', 'MODULE_PAYMENT_YANDEX_MERCHANT_PC_SHOP_ID', 'MODULE_PAYMENT_YANDEX_MERCHANT_PC_SCID', 'MODULE_PAYMENT_YANDEX_MERCHANT_PC_SECRET_KEY', 'MODULE_PAYMENT_YANDEX_MERCHANT_PC_SORT_ORDER', 'MODULE_PAYMENT_YANDEX_MERCHANT_PC_ZONE', 'MODULE_PAYMENT_YANDEX_MERCHANT_PC_ORDER_STATUS_ID', 'MODULE_PAYMENT_YANDEX_MERCHANT_PC_TEST', 'MODULE_PAYMENT_YANDEX_MERCHANT_PC_PAYMENT_TYPE');
    }
