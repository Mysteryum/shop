...
// class constructor
    function easypay() {
      ...
      $this->icon = DIR_WS_ICONS . 'easypay.png';
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_EASYPAY_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_EASYPAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_easypay_id'])) {
        $order_id = substr($_SESSION['cart_easypay_id'], strpos($_SESSION['cart_easypay_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_easypay_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_easypay_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($_SESSION['cartID'] != substr($_SESSION['cart_easypay_id'], 0, strlen($_SESSION['cartID']))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_easypay_id'] = $_SESSION['cartID'] . '-' . $insert_id;
        }
      }
    ...
    function process_button() {
      ...
      $process_button_string = '';
			...
			vam_draw_hidden_field("EP_Expires",			$ep["expiries"]).
			vam_draw_hidden_field("EP_Comment",			$ep["comment"]).
			vam_draw_hidden_field("EP_OrderInfo",		$ep["orderinfo"]).
			vam_draw_hidden_field("EP_Hash",			$ep["hash"]).
			vam_draw_hidden_field("EP_Success_URL",		$ep["success"]).
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_EASYPAY_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_EASYPAY_STATUS', 'True', '6', '3', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EASYPAY_ALLOWED', '', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EASYPAY_MERCHNO', '', '6', '5', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EASYPAY_SORT_ORDER', '0', '6', '7', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_EASYPAY_ZONE', '0', '6', '8', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EASYPAY_WEBKEY', '', '6', '9', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_EASYPAY_DEBUG', '0', '6', '10', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_EASYPAY_ORDER_STATUS_ID', '0', '6', '11', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_EASYPAY_STATUS', 'MODULE_PAYMENT_EASYPAY_ALLOWED', 'MODULE_PAYMENT_EASYPAY_MERCHNO', 'MODULE_PAYMENT_EASYPAY_SORT_ORDER', 'MODULE_PAYMENT_EASYPAY_ZONE', 'MODULE_PAYMENT_EASYPAY_WEBKEY', 'MODULE_PAYMENT_EASYPAY_DEBUG', 'MODULE_PAYMENT_EASYPAY_ORDER_STATUS_ID');
    }
